...
import { useGetProjectInvoicesQuery } from "@/redux/api/projectApi";

interface ProjectInvoicesTableProps {
  projectId?: string;
}

const ProjectInvoicesTable: React.FC<ProjectInvoicesTableProps> = ({
  projectId,
}) => {
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-[#F2F2F2]">
              <th className="p-3 md:p-4 text-[#212B36] font-inter font-normal text-[10px] md:text-xs uppercase tracking-wider text-nowrap">
                Invoice Number
              </th>
              <th className="p-3 md:p-4 text-[#212B36] font-inter font-normal text-[10px] md:text-xs uppercase tracking-wider text-nowrap">
                Due Date
          ...
          <tbody className="divide-y divide-gray-50">
            {invoicesList.map((inv, idx) => (
              <tr key={idx} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="p-3 md:p-4 text-xs md:text-sm font-inter text-[#637381] font-normal text-nowrap">
                  {inv.dueDate}
                </td>
                <td className="p-3 md:p-4 text-xs md:text-sm font-inter text-[#637381] font-normal text-nowrap">
                  {inv.amount}
                ...
                <td className="p-3 md:p-4 text-xs md:text-sm font-inter text-[#637381] font-normal">
                  {inv.dueAmount}
                </td>
                <td className="p-3 md:p-4">
                  <span
                    className={`px-2.5 py-1 rounded-md text-[10px] md:text-xs font-inter font-medium flex items-center gap-1.5 w-fit text-white ${
