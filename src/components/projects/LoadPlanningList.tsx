...
const LoadPlanningList: React.FC = () => {
  ...
  const { data, isLoading } = useGetLoadPlanningProjectsQuery({
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedIds(projects.map((p) => p.projectId));
    } else {
      setSelectedIds([]);
    }
  };
                ...
                projects.map((item, idx) => (
                  ...
                  >
                    <td className="p-3 md:p-4 text-center">
                      <input
                        type="checkbox"
                        checked={selectedIds.includes(item.projectId)}
                        onChange={(e) => handleSelectOne(item.projectId, e.target.checked)}
                        className="size-4 rounded border-gray-300 text-[#1E51A4] focus:ring-[#1E51A4]"
                      />
                    </td>
