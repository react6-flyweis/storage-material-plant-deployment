...
import { useGetPackingListDetailsQuery, type BundleItem } from "@/redux/api/shipperApi";
import { printQRCodeLabel, type QRModalData } from "@/lib/utils";

const PackingListQRLabels: React.FC = () => {
  ...
  const { data: detailsData, isLoading: isDetailsLoading } = useGetPackingListDetailsQuery(
    packingListId || "",
    { skip: !packingListId }
  );

  const packingListNo = detailsData?.packingList?.packingListNo || "N/A";
  const projectName = detailsData?.packingList?.packingListNo || "Packing List";

  // Filter labels
  const filteredLabels = useMemo(() => {
    if (!packingListId) return [];
    const bundles = detailsData?.bundles || [];
  ...
  }, [packingListId, detailsData?.bundles, searchTermLabel]);
  ...
  const handleViewDetails = (item: BundleItem) => {
    setSelectedLabel({
      ...
      shipperRef: packingListNo,
      loadId: packingListNo,
      id: item.bundleNo,
      bundleId: item._id,
    ...
    <div className="xl:pr-2 px-4 pb-10 space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-2">
        <div className="flex items-start gap-4 grow">
          ...
          <TitleSubtitle
            title={`QR Labels: ${packingListNo}`}
            subtitle="Generate, manage, and print QR labels for bundles and pallets of this packing list."
          />
        </div>
              ...
              {isDetailsLoading ? (
                ...
                <tr>
                  <td
                    ...
                    className="p-8 text-center text-gray-500 font-inter"
                  >
                    No QR Labels found for this packing list.
                  </td>
                </tr>
              ) : (
...
export default PackingListQRLabels;
